...
class HistoryWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.search_input.setPlaceholderText("搜索图号或架次号...")
        self.search_input.textChanged.connect(self._apply_filters) 
 
        # 已无结果状态列，移除结果过滤
        self.filter_combo = QComboBox()
        self.filter_combo.addItems(['全部'])
    ...
    def refresh_data(self):
        """Public method to refresh table data and re-apply filters."""
        try:
    ...
    def _delete_selected_rows(self, delete_files: bool = False):
        ...
        for r in rows:
            ...
            main_item = self.history_table.item(r, 3)
            head_item = self.history_table.item(r, 4)
            main_code = main_item.text() if main_item else ""
            head_code = head_item.text() if head_item else ""
            keys.append((timestamp, image_path, main_code, head_code))
