...
## 新增/近期解决
- 首次识别无提示音：通过 QSoundEffect 预热修复（已解决）
- 结果图未显示到结果区：修正变量名/赋值流程（已解决）
- 实时识别下画面停留上次结果：切换实时识别时取消暂停标志（已解决）
- 相机画面显示不全/被遮挡：调整容器自适应与默认窗口比例（已解决）
- 历史记录删除不移除本地图片：新增“删记录+文件”按钮（已解决）

## 已移除的功能/页面
- 上传图片、切换到图片、设置页、文本比对

## 已知问题（待优化）
- 某些驱动环境下退出时偶发 `QThread: Destroyed while thread is still running` 警告（退出阶段资源释放时机）。影响较小，后续在关闭流程中进一步确保线程优雅结束。
